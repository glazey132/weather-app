  ...
  try {
    ...
    */
    ...
    if (locationResponse.data.address.hasOwnProperty('village')) {
      console.log('location is in a village ', locationResponse.data.address.village);
      ...
      console.log('location obj has a city and village field ... find a way to handle this.', locationResponse.data.address );
    } else {
      console.log('unable to correctly parse location object thus the database search for city id did not work.');
    }
...
app.get('/getWeather/icon', async (req, res) => {
  try {
    ...
    console.log('the icon response in bakcend ', iconSrc);
    res.send(iconSrc)
  } catch(e) {
