...
function getCityWeather(city) {
  const openWeatherMapApiUrl = `https://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=${
...
function getFiveDayForecast(cityId) {
  const openWeatherMapApiUrl = `https://api.openweathermap.org/data/2.5/forecast?id=${cityId}&APPID=${
    process.env.OPEN_WEATHER_MAP_API_KEY
  }&units=imperial`;
  return axios
    .get(openWeatherMapApiUrl)
    .catch(err =>
      console.error("error calling weather 5 day forecast api", err)
    );
}
...
async function handleFiveDayForecast(fiveDayForecast) {
  ...
  let fiveDayForecastData = await [day1Obj, day2Obj, day3Obj, day4Obj, day5Obj];
